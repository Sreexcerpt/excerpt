...
import { CourseStudent, StudentList } from './model.js';
...
app.get('/api/check-invoice-number', async (req, res) => {
  try {
    console.log("Received request to check invoice number");
    const { invoiceNumber } = req.query;  // ✅ read from query param
    ...
    res.status(500).json({ message: 'Error checking invoice number' });
  }
});
  ...
  try {
    ...
    if (existingInvoice) {
      return res.status(409).json({
        ...
        message: 'This invoice number is already in use. Please use a different number.'
      });
    }
...
// Add this route in your backend (Express.js example)
...
app.get('/api/certificate/:id', async (req, res) => {
  try {
    ...
    console.error('Error generating certificate:1', error);
    res.status(500).json({ error: 'Failed to generate certificate' });
  }
});

const indstudentSchema = new mongoose.Schema({
  date: {
    type: String,
    required: true
  },
...
const Indstudent = mongoose.model('Indstudent', indstudentSchema);
...
function generateCertificate(student) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(28)
      .font('Helvetica-Bold')
      .fillColor('#0d6efd')
      .text(student.name, 0, 240, { align: 'center' });

    // Father's name
    doc.fontSize(14)
      .font('Helvetica')
      .fillColor('#666666')
      .text(`S/O ${student.fatherName}`, 0, 280, { align: 'center' });

    // College details
    doc.fontSize(14)
      .fillColor('#333333')
      .text(`from ${student.collegeName}`, 0, 310, { align: 'center' });

    // Participation text
    doc.fontSize(14)
      .text('has successfully participated in the', 0, 350, { align: 'center' });

    doc.fontSize(18)
      .font('Helvetica-Bold')
      .fillColor('#6610f2')
      .text('INDUSTRIAL VISIT', 0, 380, { align: 'center' });

    doc.fontSize(14)
      .font('Helvetica')
      .fillColor('#333333')
      .text(`organized on ${student.date}`, 0, 410, { align: 'center' });

    // Footer
    doc.fontSize(12)
      .fillColor('#666666')
      .text(`Contact: ${student.mobileNumber} | Gender: ${student.gender}`, 0, 480, { align: 'center' });

    // Signature section
    doc.fontSize(12)
      .font('Helvetica-Bold')
      .fillColor('#333333')
      .text('Authorized Signature', 150, 520);

    doc.text('Date: ' + new Date().toLocaleDateString(), doc.page.width - 250, 520);
    ...
    doc.moveTo(doc.page.width - 250, 510)
      .lineTo(doc.page.width - 150, 510)
      .stroke();
...
app.get('/api/indstudents', async (req, res) => {
  try {
    const students = await Indstudent.find().sort({ createdAt: -1 });
    res.json(students);
  } catch (err) {
...
// Get single student
app.get('/api/indstudents/:id', async (req, res) => {
  try {
    const student = await Indstudent.findById(req.params.id);
    if (!student) {
      return res.status(404).json({ error: 'Student not found' });
    }
    res.json(student);
...
app.post('/api/indstudents', async (req, res) => {
  try {
    const student = new Indstudent(req.body);
    await student.save();
...
// Update student
app.put('/api/indstudents/:id', async (req, res) => {
  try {
    const student = await Indstudent.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true, runValidators: true }
    );
...
// Delete student
app.delete('/api/indstudents/:id', async (req, res) => {
  try {
    const student = await Indstudent.findByIdAndDelete(req.params.id);
    if (!student) {
      return res.status(404).json({ error: 'Student not found' });
    }
...
app.get('/api/indcertificate/:id', async (req, res) => {
  try {
    const student = await Indstudent.findById(req.params.id);
    ...
    console.error('Error generating certificate:2', err);
    res.status(500).json({ error: 'Error generating certificate' });
  }
});

// Download all certificates as ZIP
app.get('/api/indcertificates/bulk', async (req, res) => {
  try {
    const students = await Indstudent.find();
    ...
    for (const student of students) {
      ...
      archive.append(pdfBuffer, {
        name: `certificate_${student.name.replace(/\s+/g, '_')}.pdf`
      });
    }
